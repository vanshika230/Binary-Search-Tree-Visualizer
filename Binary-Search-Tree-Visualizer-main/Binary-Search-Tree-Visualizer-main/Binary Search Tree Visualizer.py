...
class BST:
    def __init__(self, root):
        ...
        self.make_canvas = Canvas(self.window,width=1160,height=520,bg="#3A4F7A",relief=RAISED,bd=8)
        self.make_canvas.place(x=0,y=0)
        # Canvas make for result presentation
        self.result_canvas = Canvas(self.window,width=1340,height=168,bg="#FFC6D3",relief=RAISED,bd=4)
    ...
    def heading_root_and_null_make(self):# By default window presentation
        make_heading = Label(self.make_canvas,text="Binary Search Tree Visualizer",font=("Arial",27,"bold","italic"),fg="black",bg="#39B5E0")
        make_heading.place(x=350,y=10)
        make_root = Label(self.make_canvas,text="Root",font=("Arial",17,"bold"),fg="black",bg="#39B5E0")
        ...
        self.make_canvas.create_polygon(root_indicator, width=3, fill="#39B5E0", outline="black")

        self.make_null = Label(self.make_canvas,text="NULL",font=("Arial",17,"bold","italic"),fg="#2E37FE",bg="#39B5E0")
        self.make_null.place(x=565,y=90+50)

    def make_instructional_buttons(self):# For make Instructional buttons
        #Insert button
        self.insert_node = Button(self.window,text="Insert",font=("Arial",20,"bold"),bg="#E98EAD",fg="black",relief=RAISED,bd=8,command=lambda: self.take_input_set_up(1),state=NORMAL)
        self.insert_node.place(x=1210,y=10)

        #LabelFrame make for Traversing Buttons
        make_label_frame_for_traversing = LabelFrame(self.window,text="Traversing",width=50,bg="#FF8E9E",font=("Arial",13,"bold"),fg="black")
        make_label_frame_for_traversing.place(x=1183,y=180)

        self.pre_order_traversal  = Button(make_label_frame_for_traversing,text="Pre-order",font=("Arial",10,"bold"),bg="black",fg="#FF8E9E",relief=RAISED,bd=8,command=lambda: self.traversing_decision_making(1),state=NORMAL)
        self.pre_order_traversal.pack(padx=10,pady=5)

        self.in_order_traversal = Button(make_label_frame_for_traversing, text="In-order", font=("Arial", 10, "bold"),bg="black", fg="#FF8E9E", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(2), state=NORMAL)
        self.in_order_traversal.pack(padx=10, pady=5)

        self.post_order_traversal = Button(make_label_frame_for_traversing, text="Post-order", font=("Arial", 10, "bold"), bg="black", fg="#FF8E9E", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(3), state=NORMAL)
        self.post_order_traversal.pack(padx=10, pady=5)

        self.level_order_traversal = Button(make_label_frame_for_traversing, text="Level-order Display", font=("Arial", 10, "bold"), bg="black", fg="#FF8E9E", relief=RAISED, bd=8, command=lambda: self.traversing_decision_making(4), state=NORMAL)
        ...
        self.level_order_display = Button(make_label_frame_for_traversing, text="Level-order Concept",
                                            font=("Arial", 10, "bold"), bg="black", fg="#FF8E9E", relief=RAISED, bd=8,
                                            command=self.level_order_concept_by_queue, state=NORMAL)
        self.level_order_display.pack(padx=3, pady=5)
        # Delete Button
        self.make_delete = Button(self.window, text="Delete", font=("Arial", 16, "bold"), bg="black", fg="#FF8E9E", relief=RAISED, bd=8, command=lambda: self.take_input_set_up(2), state=NORMAL)
    ...
    def take_input_set_up(self,indicator):# Set up for take input value for node insert and delete
        ...
        self.take_input = Label(self.window, text="Enter the number to input", fg="blue", bg="#FF8E9E", font=("Arial", 10, "bold"))
    ...
    def make_default_node_with_set_position(self):# For node make and set at right position in canvas
        try:
            ...
            self.make_node = self.make_canvas.create_oval(505, 85+50, 555, 135+50, width=2, fill="#F56EB3", outline="#F56EB3")
            self.make_label= Label(self.window, text=self.take_entry.get(), fg="black", bg="#F56EB3", font=("Arial", 12, "bold"))
    ...
    def decision_making_about_left_side_direction(self):
        try:
            ...
            make_greater_than_label = Label(self.window, text=">", fg="black", bg="#FF8E9E", font=("Arial", 20, "bold"))
    ...
    def decision_making_about_right_side_direction(self):
        try:
            self.right_activate = 1
            self.left_activate = 0
            make_less_than_label = Label(self.window, text="<", fg="black", bg="#FF8E9E", font=("Arial", 20, "bold"))
    ...
    def make_container_in_result_canvas(self):# In down canvas
        self.traversing_label = Label(self.result_canvas,bg="#FF8E9E",fg="black",text="Traversing Result",font=("Arial",15,"bold","italic"))
        self.traversing_label.place(x=480,y=10)
        distance_maintainer = 8
        for i in range(15):
            self.value_show.append(i)
            self.value_show[i] = Label(self.result_canvas,bg="#FF8E9E",fg="black",text=" ",width="4",height="1",font=("Arial",15,"bold","italic"),relief=SUNKEN,bd=5)
            self.value_show[i].place(x= distance_maintainer,y=50)
            distance_maintainer+=91

        make_status = Label(self.result_canvas,bg="#FF8E9E",fg="black",text="Status",font=("Arial",17,"bold","italic"))
        make_status.place(x=580,y=100)

        self.status_note = Label(self.result_canvas,bg="#FF8E9E",fg="black",text="Play With BST",font=("Arial",20,"bold","italic"))
    ...
    def left_none_right_exist(self,node):
        ...
        if left_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                make_circle = self.make_canvas.create_oval(node_coord, width=3, fill="#FF8E9E", outline="black")
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="black", bg="#FF8E9E",
                                   font=("Arial", 12, "bold"))
    ...
    def right_none_left_exist(self, node):
        ...
        if  right_side_existence_checking_of_one_side_down_label_node == 1:
            ...
            if temp1:
                ...
                label_make = Label(self.window, text=take_temp1_val, fg="black", bg="#FF8E9E", font=("Arial", 12, "bold"))
                label_make.place(x=node_coord[0] + 14, y=node_coord[1] + 14)
    ...
    def color_indicator_to_delete(self,node,temp):# Color indicating to deleted and replaced node
        ...
        self.make_canvas.itemconfig(temp[0], fill="#FF8E9E", outline="black")
        temp[1].config(fg="black", bg="#FF8E9E")
        self.make_canvas.itemconfig(node[0], fill="#FF8E9E", outline="black")
        node[1].config(fg="black", bg="#FF8E9E")
        self.status_note.config(text="Work done")
        self.status_note.place(x=550, y=130)
